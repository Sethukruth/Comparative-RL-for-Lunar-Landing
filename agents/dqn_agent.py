...
Implements:
  ...
  • Experience replay (uniform buffer)
  • Periodic hard target-network updates
  • TensorBoard logging of loss, reward, and epsilon
"""
...
log = get_logger("DQNAgent")


class DQNAgent:
    """Standard DQN agent with target network.
    ...
    def __init__(
        ...
        tb_log_dir: str = "tensorboard_logs/dqn",
    ) -> None:
    ...
    def act(self, state: np.ndarray, eval_mode: bool = False) -> int:
        ...
        Args:
            ...
            eval_mode: If True, always exploit (no randomness).
    ...
    def learn(self) -> Optional[float]:
        """Perform a single gradient step on a replayed mini-batch.
        ...
        # Target Q-values (standard DQN: max over target net)
        with torch.no_grad():
    ...
    def decay_epsilon(self) -> None:
        """Apply multiplicative epsilon decay (clamped at ``epsilon_min``)."""
    ...
    def save(self, path: str) -> None:
        """Save policy-net weights to disk."""
        os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
        torch.save(self.policy_net.state_dict(), path)
        log.info(f"Model saved -> {path}")

    def load(self, path: str) -> None:
        """Load policy-net weights from disk and sync target net."""
